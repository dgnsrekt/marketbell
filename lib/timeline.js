...
export function makeWeekendStrip(getModel) {
    ...
    area.connect('repaint', () => {
        ...
        rgba(cr, [1, 1, 1], 0.14);
        for (const f of m.grid) {
            cr.rectangle(Math.round(f * w), 0, 1, h); cr.fill();
        }
        cr.restore();

        marker(cr, GREEN, m.openFrac * w, w, h);
        marker(cr, AMBER, m.nowFrac * w, w, h);
...
function marker(cr, color, x, w, h) {
    x = Math.max(1, Math.min(Math.round(x), w - 2));
    rgba(cr, color, 1);
    cr.rectangle(x, 0, 2, h); cr.fill();
    cr.arc(x + 1, 4, 3, 0, 2 * Math.PI); cr.fill();
}
