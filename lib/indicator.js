...
function legendRow() {
    ...
    row.add_child(new St.Label({ text: '★', style_class: 'marketbell-star', style: 'padding-right:0;', y_align: CENTER }));
    row.add_child(new St.Label({ text: _('primary'), y_align: CENTER }));
    return item;
}
...
class MarketBellIndicator extends PanelMenu.Button {
    ...
    update() {
        ...
        if (markets.length === 0) {
            ...
            if (anyOpen) {
                // Open markets first (stable sort keeps watched order within each group)
                const openFirst = states.slice().sort((a, b) =>
                    (a.st?.isOpen ? 0 : 1) - (b.st?.isOpen ? 0 : 1));
                for (const { m } of openFirst)
