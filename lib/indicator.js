...
import { marketState, sessionSegments, localNow, nextOpenParts, humanize } from './marketclock.js';
...
function liveRow(m, now, isPrimary) {
...
function closedRow(m, now, isPrimary) {
    const { item, row } = menuRow();
...
function closedBanner(soonestSecs) {
    const item = new PopupMenu.PopupBaseMenuItem({ reactive: false, can_focus: false });
    ...
    top.add_child(new St.Label({ text: _('opens in %s').format(humanize(soonestSecs)), style_class: 'marketbell-weekcount', y_align: CENTER }));
    box.add_child(top);
    box.add_child(makeWeekendStrip());
    item.add_child(box);
    return item;
}
        ...
        if (markets.length === 0) {
            ...
            if (anyOpen) {
                for (const m of markets)
                    this._marketSection.addMenuItem(liveRow(m, now, m.id === primaryId));
            } else {
                let soonest = Infinity;
                for (const { st } of states)
                    if (st) soonest = Math.min(soonest, st.openDt.to_unix() - now.to_unix());
                this._marketSection.addMenuItem(closedBanner(Number.isFinite(soonest) ? soonest : 0));
                for (const m of markets)
                    this._marketSection.addMenuItem(closedRow(m, now, m.id === primaryId));
            }
