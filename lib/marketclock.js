...
export function marketState(market, now) {
    ...
    for (let offset = 0; offset <= 8; offset++) {
        ...
        // Half-day (early-close) trading days override the normal close time;
        // everything downstream (countdown, segments, close bell) derives from
        // closeDt, so the early close just flows through.
        const closeHM = market.earlyCloses?.[dateKey(openDt)] ?? market.close;
        const closeDt = GLib.DateTime.new(tz, y, mo, d, closeHM[0], closeHM[1], 0);
...
// Time to the governing bell (close if open, else next open) — for the popup's
// click-to-test notification. Pure data; the indicator wraps it in translated
// text. -> { isOpen, seconds, localHHMM } or null (no trading day in range).
export function nextBell(market, now) {
    const st = marketState(market, now);
    if (!st)
        return null;
    const bell = st.isOpen ? st.closeDt : st.openDt;
    return {
        isOpen: st.isOpen,
        seconds: bell.to_unix() - now.to_unix(),
        localHHMM: bell.format('%H:%M'),
    };
}
